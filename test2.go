...
func testmain() {
	...
	// Make a connection to Mongodb Instance
	session_to_get_number_of_builds, err_session := mgo.Dial(dbUrl)
	if err_session != nil {
		panic(err_session)
	}
	// Declare 'defer' and make 'Close()' will be called after this statement block is finished
	defer session_to_get_number_of_builds.Close()
	// Get DB instance with dbName
...
func main() {
	buildLogFile := "/mnt/work/temp/jobs/starfish-webos4tv-verify-k7hp/builds/lastSuccessfulBuild/log"
	buildLog, err := os.Open(buildLogFile)
	if err == nil {
		buildLogReader := bufio.NewReader(buildLog)
		var (
